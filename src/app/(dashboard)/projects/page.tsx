...
import { Plus, MoreVertical, Search, MonitorPlay as Youtube, LayoutGrid, List } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-glow">Projects</h1>
          <p className="text-muted-foreground mt-1">Manage your content channels and brands.</p>
        </div>
          ...
          <DialogContent className="sm:max-w-[425px] glass border-white/10">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input id="name" placeholder="e.g. Tech Channel" className="bg-background/50" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="platform">Platform</Label>
                <Input id="platform" defaultValue="YouTube" className="bg-background/50" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="category">Category</Label>
                <Input id="category" placeholder="e.g. Technology, Education" className="bg-background/50" />
              </div>
            </div>
            <DialogFooter>
      ...
      {filteredProjects.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center glass rounded-2xl border-dashed border-2 border-white/10">
          ...
          <p className="text-muted-foreground mb-6 max-w-sm">
            You don't have any projects matching your search. Create a new one to get started.
          </p>
        ...
        <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" : "flex flex-col gap-4"}>
          {filteredProjects.map((project, i) => (
            ...
            >
              ...
              <div className={`relative z-10 ${viewMode === "list" ? "flex-1" : "flex-1"}`}>
                ...
                <div className="flex items-center gap-3 mt-auto">
                  ...
                  <span className="text-xs text-muted-foreground flex items-center gap-1">
                    <span className="font-medium text-foreground">{project.videosCount}</span> videos
                  </span>
                </div>
              </div>
            </motion.div>
